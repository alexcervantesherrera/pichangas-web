...
export default function Crear() {
  ...
  const [form, setForm] = useState({
    nombre: '', fecha: '', distrito: '',
    lat: '', lng: '', publica: true, capacidad: 12,
  })
  ...
  async function geocode() {
    ...
    try {
      ...
      if (data.length === 0) {
        setGeoMsg('No se encontró el distrito. Ingresa las coordenadas manualmente.')
        return
      }
      setForm(prev => ({ ...prev, lat: data[0].lat, lng: data[0].lon }))
      setGeoMsg(`✓ Coordenadas encontradas para ${data[0].display_name.split(',')[0]}`)
    } catch {
      setGeoMsg('Error al buscar coordenadas. Verifica tu conexión.')
    } finally {
      setGeocoding(false)
    }
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      const p = await api.post('/pichangas', {
        nombre:    form.nombre,
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
      <h1 className="text-xl font-bold mb-6">Nueva pichanga</h1>
      <form onSubmit={handleSubmit} className="card space-y-4">
        ...
        <div>
          ...
          <input type="text" required className="input" value={form.nombre}
            onChange={e => set('nombre', e.target.value)} placeholder="ej. Pichanga del sábado" />
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            <button
              type="button"
              onClick={geocode}
              disabled={geocoding || !form.distrito.trim()}
          ...
          {geoMsg && (
            <p className={`text-xs mt-1 ${geoMsg.startsWith('✓') ? 'text-green-600' : 'text-amber-600'}`}>
              {geoMsg}
            </p>
          )}
        </div>
        ...
        <p className="text-xs text-gray-400 -mt-2">Usa "Buscar" para rellenar automáticamente desde el distrito.</p>
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button type="submit" className="btn-primary flex-1" disabled={loading}>
            {loading ? 'Creando...' : 'Crear pichanga'}
          </button>
        </div>
      </form>
    </div>
